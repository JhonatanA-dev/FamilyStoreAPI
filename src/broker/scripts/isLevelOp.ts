...
interface isLevelOpReturninterface {
 ...
 export function isLevelOp({level,xp,taskDifficulty}:isLevelOpinterface):isLevelOpReturninterface {
    ...
    if (taskDifficulty === 1) add = 50
    if (taskDifficulty === 2) add = 100
    if (taskDifficulty === 3) add = 200
    ...
    if (xpRegister >= xpn) return { levelUP:true, xp : Math.round(xpRegister)}

    return { levelUP:false,xp : Math.round(xpRegister)  }
}
